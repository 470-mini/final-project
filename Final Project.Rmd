...
After data cleaning, the current total observations would be used in the following EDA and models are 4234. 
...
```{r}
ggplot(google.part, aes(x = ForP, group = Category, color = Category,fill = Category)) + geom_bar() + xlab("Free or Paid")
```
...
Rating is left-skewed and Size is right-skewed. Need to log Size in the following models.
