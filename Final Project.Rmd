...
It's clear that if we group the apps by their categories, the relationship between Price and Rating will be different based on their category. For Fitness, Media, and Tools apps, their price have negative relationship with Rating, higher price will have lower Rating for these three kinds of apps, especially for Media, and Tools apps, this negative relationship is stronger. However, for Family and Game, their Price have weak positive relationship with Rating, the increasing of Price will slightly increase the Rating of apps. Thus, we also need to include the category in the model if we want to include the Price, it will be not sufficient enought if we only include the Price in the model.
...
```{r}
ggplot(data = google.part%>%filter(Type == "Paid"), aes( x = Price, y = Rating)) + geom_point() + geom_smooth(method = "lm", color = "blue",se = FALSE)+ xlab("Price ($)") + geom_smooth(aes(x = Price, y = Rating, weight = Reviews), se = FALSE, color = "red", method = "lm")
```
