...
```{r}
ggplot(data = google.part, aes( x = Rating.range, group = Type)) + geom_bar(show.legend = FALSE,aes(y = ..prop.., fill = factor(..x..)), stat = "count") + geom_text(aes(label = scales::percent(..prop..), y = ..prop..), stat = "count") + facet_wrap(~Type) + labs(fill = "Rating range", title = "Percentage of different rating range within Free/Paid Apps") + cbp()+ ylab("Percentage")+scale_y_continuous(labels=scales::percent)
...
grid.arrange(type, category, ratingR, ncol = 3)
```
